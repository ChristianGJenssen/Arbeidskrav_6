...
I lang tid har man vært enige om at styrketrening er den mest optimale måten og øke muskelmasse og styrke. Men hva som er den mest optimale metoden og som gir best resultat er edna kraftig diskutert. I Krieger sin artikkel fant han ikke enn mer singinfikant endring jo høyere antall sets. (@Krieger2010) Så ser man på studiet til Santana så mener de at jo større mengde desto større effekt. (@Santana2021)

Så i denne artikelen her skal vi se på resultatene til et studie som har sett på hvor stor effekt har et set vs tre set hos utrente deltakere. I læreboka styrketrening blir det nevnt at når en utrent person begynner å trene strukurert styrketrening så er det forventa og se økning i styrke på me 0.5-3% etter hver økt.(@raastad2010) Ifølge studien til Wernebom så kan man også forvente å se en økning på 0.1 til 0.5% i tverssnitt på muskulaturen.(@Wernebom2012)
...
#| fig-cap: Tabell 1. Tabell over deltakere PS. ser den er feil men får ikke til å få den til å ta riktig antall
...
# Plot for multiple sets with average line
...
ggplot(str_single, aes(time, load, group = participant, color = participant)) + 
  geom_line() + 
  stat_summary(fun = mean, geom = "line", aes(group = 1), color = "black", linetype = "dashed", size = 1) + # Øk størrelsen (size) etter behov
  facet_wrap(~ exercise, scales = "free") +
  ggtitle("Single Set")
...
result_table_legext <- data.frame(
  ...
  p_value = sprintf("%.3f", t_test_legext$p.value),
